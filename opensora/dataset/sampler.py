...
class VariableVideoBatchSampler(DistributedSampler):
    ...
    def group_by_bucket(self) -> dict:
        ...
        return bucket_sample_dict  # {BUCKET_ID_TUPLE:[DATASET_INDEX, ...], ...}

    def get_num_batch(self) -> int:
        """ Get batches count, called before the training loop. """
    ...
    def __iter__(self) -> Iterator[List[str]]:
        ...
        bucket_micro_batch_count = OrderedDict()    # {BUCKET_ID_TUPLE: NUM_MICRO_BATCHES, ...}
        ...
        for bucket_id, data_list in bucket_sample_dict.items():  # {BUCKET_ID_TUPLE:[DATASET_INDEX, ...], ...}
            ...
            if remainder > 0:
                if not self.drop_last:  # drop last for each bucket_id
        ...
        bucket_id_access_order = []  # [BUCKET_ID_TUPLE,BUCKET_ID_TUPLE,...]
    ...
    def _print_bucket_info(self, bucket_sample_dict: dict, verbose=True) -> None:
        ...
        for k, v in bucket_sample_dict.items():  # {BUCKET_ID_TUPLE:[DATASET_INDEX, ...], ...}
            size = len(v)
            total_samples += size
            num_dict[k] = size
